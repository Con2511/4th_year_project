...
void loop() {
  ...
  if (read==1 and x==0){
    ...
    //display=1*decode[9]+2*decode[8]+4*decode[7]+8*decode[6]+16*decode[5]+32*decode[4]+64*decode[3]+128*decode[2]+256*decode[1]+512*decode[0];
    
    //Serial.println(decode[0]);
    //Serial.println(" ");
    //for(int i=0;i<10;i=i+1){
    //Serial.print(decodetemp[i]);
    //Serial.println(decodetemp[i]);
    //}
    //Serial.println(" ");
    ...
    digitalWrite(9,0);
    delay(700);             //Maybe need to tone down this delay? or add foot to ensure no run on
  }
  else{
    //T=analogRead(A0);
    digitalWrite(9,0);
