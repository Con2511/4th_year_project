...
void setup() {
  ...
  yr=now.year()-2000;
  sprintf(buf1, "%02d_%02d_%02d.txt", now.day(), now.month(), yr); //+ now.month()+now.year();
  //buf1=buf1+".txt";
  //date.concat("/");
  lcd.setCursor(0, 1);
  lcd.print(buf1);
  //delay(1000);
  myFile= SD.open(buf1, FILE_WRITE);
  //pinMode(A0,INPUT);
  if(myFile){
    digitalWrite(9,0);
...
void loop() {
  ...
  if(1011==header[0]*1000+header[1]*100+header[2]*10+header[3]*1 or 1011==header2[0]*1000+header2[1]*100+header2[2]*10+header2[3]*1){
    x=0;
    review=0;
    //digitalWrite(9,1);
    fourth=store;
    ...
    if (tally==3){
      ...
      sprintf(buf2, "%02d/%02d/%02d ", now.day(), now.month(), yr);
